...
import { Menu, X, ShoppingBag, LogIn, LogOut, LayoutDashboard, Package, User, ChevronLeft, Home } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-2">
        {!isHome && (
          <button onClick={goBack} aria-label="back"
            className="md:hidden p-2 rounded-full bg-white shadow active:scale-90 transition">
            <ChevronLeft className="w-5 h-5 text-ilkal-maroon" />
          </button>
        )}
        <Link to="/" className="flex items-center gap-2 mr-auto" onClick={() => setOpen(false)}>
          <div className="w-10 h-10 rounded-full bg-white grid place-items-center shadow-md ring-1 ring-ilkal-gold/40 overflow-hidden">
            <img src={logo} alt="Ilkal Kart" className="w-full h-full object-contain" />
          </div>
          <div className="leading-tight">
            <div className="font-serif text-xl text-ilkal-maroon font-bold">Ilkal Kart</div>
            <div className="text-[10px] tracking-widest text-ilkal-gold -mt-1">PURE • AUTHENTIC • GENUINE • ELEGANCE</div>
          </div>
        </Link>
        ...
        <div className="flex items-center gap-2 ml-auto">
          {!isHome && (
            <Link to="/" aria-label="home" className="md:hidden p-2 rounded-full bg-white shadow active:scale-90 transition">
              <Home className="w-5 h-5 text-ilkal-maroon" />
            </Link>
          )}
          <Link to="/checkout" className="relative p-2 rounded-full bg-white shadow hover:shadow-lg transition">
            <ShoppingBag className="w-5 h-5 text-ilkal-maroon" />
            {count > 0 && (
              <span className="absolute -top-1 -right-1 bg-ilkal-gold text-ilkal-deep text-[10px] font-bold w-5 h-5 grid place-items-center rounded-full shadow">
                {count}
              </span>
            )}
          </Link>

          <button className="md:hidden p-2 rounded-full bg-white shadow" onClick={() => setOpen(o => !o)} aria-label="menu">
            {open ? <X className="w-5 h-5 text-ilkal-maroon" /> : <Menu className="w-5 h-5 text-ilkal-maroon" />}
          </button>

          {!user ? (
            <div className="flex items-center gap-2 pl-2 ml-1 border-l border-ilkal-gold/30">
            ...
            <div className="relative pl-2 ml-1 border-l border-ilkal-gold/30" ref={profileRef}>
